...
<script type="text/javascript">
	function checking(target,id){
		if(target == 2){
			if(confirm("确定审核通过？")){
				window.location.href="commitWait?target=2&id="+id;
			}
		}else{
			if(confirm("确定审核不通过？")){
				window.location.href="commitWait?target=4&id="+id;
			}
		}
	}
</script>
...
<body>
<div style="padding-left:50px;padding-right:230px;"><font size="6" color="blue">待审核订单信息列表：</font><br /></div>
<table align="center">
	<tr>
	...
	<s:iterator id="viewOrder" status="st" value="viewOrderList">
		...
		<tr>
			...
			<td>${userName}</td>
			<td>${stateDes}</td>
			<td><input type="button" value="审核通过" onclick="checking(2,'${id}');"/>
			<input type="button" value="审核不通过" onclick="checking(4,'${id}');"/></td>
		</tr>
	</s:iterator>
</table>
